...
def get_mask_card_number(card_number: int) -> str:
    ...
    if type(card_number) is not int:
        logger.error(f"Функция get_mask_card_number: передано не число: {card_number}")
        raise TypeError("Type error. Please use int type.")
    if card_number < 1000000000000000 or card_number > 9999999999999999:
        logger.error(f"Функция get_mask_card_number: передано число содержащее не 16 цифр: {card_number}")
...
def get_mask_account(account_number: int) -> str:
    ...
    if type(account_number) is not int:
        logger.error(f"Функция get_mask_account: передано не число: {account_number}")
        raise TypeError("Type error. Please use int type.")
    if account_number < 10000000000000000000 or account_number > 99999999999999999999:
        logger.error(f"Функция get_mask_account: передано число содержащее не 20 цифр: {account_number}")
