...
def filter_by_description(transactions: list[dict], search_str: str) -> list[dict]:
    ...
    return [
        ...
        if re.search(search_str.lower(), transaction.get("description", "").lower())
    ]


def category_counter(transactions: list[dict], counters: list) -> dict:
    """Функция подсчета количества указанных категорий"""
    types_transactions = [
        transaction.get("description") for transaction in transactions if transaction.get("description") in counters
    ]
    return dict(Counter(types_transactions))
